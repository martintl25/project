...
// my add
/*
static void set_freq(int pid, int oomadj){
    char adj_path[80];
    char pre_oomadj[20];
    char freq_path[80];
    char pre_freq[20];
    ...
    snprintf(adj_path, sizeof(adj_path), "/proc/%d/oom_score_adj", pid);
    int fd = open(adj_path, O_RDONLY | O_CLOEXEC);
    if (fd < 0) {
        ALOGE("Error opening %s; errno=%d; freq", adj_path, errno);
        return;
    }
    memset(pre_oomadj, 0, sizeof(pre_oomadj));
    ret = read(fd, pre_oomadj, 20);
    if (ret < 0){
        ALOGE("Error reading %s; errno=%d; freq", adj_path, errno);
    }
    close(fd);

    val_int = strtol(pre_oomadj, NULL, 0);

    // if (val_int <= oomadj)
    if (!(val_int >= 900 || val_int == 700))
        return ;

    snprintf(freq_path, sizeof(freq_path), "/proc/%d/freq", pid);
    fd = open(freq_path, O_RDONLY | O_CLOEXEC);
    if (fd < 0) {
        ALOGE("Error opening %s; errno=%d; freq", freq_path, errno);
        return;
    }
    memset(pre_freq, 0, sizeof(pre_oomadj));
    ret = read(fd, pre_freq, 20);
    if (ret < 0){
        ALOGE("Error reading %s; errno=%d; freq", freq_path, errno);
    }
    close(fd);

    val_int = strtol(pre_freq, NULL, 0);
    if (val_int + 1 > 100)
        val_int = 100;
    else
        val_int += 1;
    
    snprintf(path, sizeof(path), "/proc/%d/freq", pid);
    snprintf(val, sizeof(val), "%d", val_int);
    writefilestring(path, val);
}

static bool need_to_update(int pid, int oomadj){
    char adj_path[80];
    char pre_oomadj[20];
    int val_int;
    int ret;
    ...
    int fd = open(adj_path, O_RDONLY | O_CLOEXEC);
    if (fd < 0) {
        ALOGE("Error opening %s; errno=%d; CPS", adj_path, errno);
        return false;
    }
    memset(pre_oomadj, 0, sizeof(pre_oomadj));
    ret = read(fd, pre_oomadj, 20);
    if (ret < 0){
        ALOGE("Error reading %s; errno=%d; CPS", adj_path, errno);
    }
    close(fd);
...
static void set_service_child_parent(int pid, int oomadj){
    if (oomadj < 900)
        return ;

    if (!need_to_update(pid, oomadj))
        return ;
    ...
    char val[20];
    if (p == 0){
        execl("/system/bin/sh", "sh", "/data/local/tmp/am.sh", NULL);
        ALOGE("sh error\n");
        exit(0);
    }
    else{
        int w = wait(NULL);
        int cp_pid = -1, sr_pid = -1;
        FILE *fd = fopen("/data/local/tmp/sr_res.txt", "r");
        if (fd == NULL)
            ALOGE("Open /data/local/tmp/sr_res.txt\n");
        else{
            char buff[256];
            int buff_size = 256;
            // read child parent
            if (fgets(buff, buff_size, fd) != NULL){
                ...
                int n = 0; // next n line is child parent;
                for (int i = 0; i < len; i++){
                    n *= 10;
                    n += buff[i] - '0';
                }
            ...
            fclose(fd);
            if (sr_pid == 1){
                snprintf(path, sizeof(path), "/proc/%d/service", pid);
                snprintf(val, sizeof(val), "%d", sr_pid);
                writefilestring(path, val);
            }
            else{
                sr_pid = 0;
            ...
            if (cp_pid != -1){
                snprintf(path, sizeof(path), "/proc/%d/parentchild", pid);
                snprintf(val, sizeof(val), "%d", cp_pid);
                writefilestring(path, val);
            }
        }
...
static void cmd_procprio(int pid, int uid, int oomadj, int freq, int restart) {
    struct proc *procp;
    char path[80];
    char val[20];
 ...
 //   set_freq(pid, oomadj);
    ...
    // my add
//    set_service_child_parent(pid, oomadj);
    ...
    // my add
    snprintf(path, sizeof(path), "/proc/%d/service", pid);
    snprintf(val, sizeof(val), "%d", restart);
    writefilestring(path, val);
...
static void ctrl_command_handler(void) {
    ...
    case LMK_PROCPRIO:
        ...
        if (nargs != 5)
            goto wronglen;
        cmd_procprio(ntohl(ibuf[1]), ntohl(ibuf[2]), ntohl(ibuf[3]), ntohl(ibuf[4]), ntohl(ibuf[5]));
