...
final class ProcessList {
    ...
    static final int CACHED_APP_MAX_ADJ = 990; // my add
     ...
     */
     // my add
     // change interface, add freq, and restart service
    //public static final void setOomAdj(int pid, int uid, int amt) {
    public static final void setOomAdj(int pid, int uid, int amt, int freq, boolean restart) {
        if (amt == UNKNOWN_ADJ)
            return;

        long start = SystemClock.elapsedRealtime();
        ByteBuffer buf = ByteBuffer.allocate(6 * 4);
